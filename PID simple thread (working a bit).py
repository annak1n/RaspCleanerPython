...
class Control_robo:
    ...
    def background_2(self):
        ...
        def direction(self, error_z, diff_z, sum_z):
            ...
            if self.select == 's':
                if error_z > 0 and self.TARGET_2 > 80:
                    self.TARGET_2 = self.TARGET_2 + (error_z * KP) + (diff_z * KD) + (sum_z * KI) #DISCART SUM TO TRANSITIONS
                    print("codiçao 1")
                elif error_z > 0 and self.TARGET_2 <= 80:
                    self.TARGET_2 = self.TARGET_1 + (error_z * KP) + (diff_z * KD) #+ (sum_z * KI)
                    print("condicao 2")
                elif error_z < 0 and self.TARGET_2 <= 80:
                    self.TARGET_2 = self.TARGET_2 + (error_z * KP) + (diff_z * KD) + (sum_z * KI) #DISCART SUM TO TRANSITIONS
                    print("condicao 3")
        ...
        def focus(self):
            ...
            p_focus = multiprocessing.Process(target=read_thread, args =(self,))
            p_focus.daemon = True
            p_focus.start()
            p_focus_Alive = True

            #p_focus.terminate()
            while p_focus_Alive == True:
                ...
                if self.focus >-8 and self.focus<8:
                    p_focus_Alive = False
                time.sleep(1)
            if p_focus_Alive == False:
                p_focus.terminate()
                return True

        def read_thread(self):
            while True:
                #print("To no processo")
                angle_data = self.gyro.reading()
                self.focus = angle_data['z']
                print(self.focus)
